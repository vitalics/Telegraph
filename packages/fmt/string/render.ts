...
/**
 * The `render()` method returns a formatted string using the first argument as a printf-like format string which can contain zero or more format specifiers. Each specifier is replaced with the converted value from the corresponding argument. Supported specifiers are:
 * 
 ...
 * ```
 * 
 * If there are more arguments passed to the `render()` method than the number of specifiers, the extra arguments are concatenated to the returned string, separated by spaces:
 * ``` ts
 * util.format('%s:%s', 'foo', 'bar', 'baz');
 * // Returns: 'foo:bar baz'
 * ```
 * 
 * If only one argument is passed to `render()`, it is returned as it is without any formatting:
 * ``` ts
 * util.format('%% %s');
 * // Returns: '%% %s'
 * ```
 * `render()` is a synchronous method that is intended as a debugging tool. Some input values can have a significant performance overhead that can block the event loop. Use this function with care and never in a hot code path.
 *
 ...
 */
export default function render<S extends string>(str: S | TemplateStringsArray, ...args: ParsePrintFormat<S>): [string, ExtraReplyMessage] {
