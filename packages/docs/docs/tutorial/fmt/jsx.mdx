...
import { render, Bold, Break, Italic } from "@tlgr/fmt/default";
...
- Use components only for your reply context. For `reply` use components from `default` folder.
  For `replyWithHTML` use components from `html` folder.
  for `replyWithMarkdown` and `replyWithMarkdownV2` use components from `markdown`.
